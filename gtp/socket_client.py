...
def respond_success(response: str, ongoing: bool = False) -> NoReturn:
    ...
    client_socket.send((f"={gtp_command_id} " + response + terminator).encode('utf-8'))
...
def respond_failure(response: str) -> NoReturn:
    ...
    client_socket.send((f"={gtp_command_id} " + response).encode('utf-8'))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            net = data["net"]
            client_socket.send('init_success'.encode('utf-8'))
            break
        except KeyError:
            client_socket.send('!!!init_error'.encode('utf-8'))

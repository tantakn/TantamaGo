...
class PolicyHead(nn.Module):
    ...
    def __init__(self, board_size: int, channels: int, momentum: float=0.01):
        ...
        self.conv_layer = nn.Conv2d(in_channels=channels, out_channels=2, \
            kernel_size=1, padding=0, bias=False, dtype=float)
        self.bn_layer = nn.BatchNorm2d(num_features=2, eps=2e-5, momentum=momentum, dtype=float)
        self.fc_layer = nn.Linear(2 * board_size ** 2, board_size ** 2 + 1, dtype=float)
        self.relu = nn.ReLU()

...
def train_on_gpu_endless(program_dir: str, board_size: int, batch_size: int, \
    epochs: int, network_name: str, npz_dir: str = "data", checkpoint_dir: str = None) -> None: # pylint: disable=R0914,R0915
    """教師あり学習を実行し、学習したモデルを保存する。
    ...
    while True:
        ...
        if not os.path.isfile(train_npz_path):
            print(f"👺File not found: {train_npz_path}")
            sys.exit()

        plane_data, policy_data, value_data = load_data_set(train_npz_path)
        ...
        print_learning_process(train_loss, 0, npz_cnt, iteration, epoch_time)

        npz_cnt += 1
        # if True:################
        if npz_cnt % 10 == 0:###############
            dual_net_copy = copy.deepcopy(dual_net)######
            torch.save(dual_net_copy.to("cpu"), os.path.join("model", f"sl-model_{dt_now.strftime('%Y%m%d_%H%M%S')}_{npz_cnt:0>3}.bin"))
            # save_model(dual_net_copy, os.path.join("model", f"sl-model_{dt_now.strftime('%Y%m%d_%H%M%S')}_Ep:{epoch:0>2}.bin"))######epoch毎に保存
            ...
            torch.save({
                ...
                }, os.path.join("model", f"checkpoint_{dt_now.strftime('%Y%m%d_%H%M%S')}_{npz_cnt:0>4}.bin"))
        
            print(f"🐾model and checkpoint {dt_now.strftime('%Y%m%d_%H%M%S')}_{npz_cnt:0>4}.bin saved.")###############
        ...
        # for data_index, train_npz_path in enumerate(train_npz_paths):
            ...
            if int(rank) == 0:
                ...
                if npz_cnt % 2 == 0:###############
                    ...
                    print(f"🐾model and checkpoint {dt_now.strftime('%Y%m%d_%H%M%S')}_{npz_cnt:0>3}.bin saved.")###############
